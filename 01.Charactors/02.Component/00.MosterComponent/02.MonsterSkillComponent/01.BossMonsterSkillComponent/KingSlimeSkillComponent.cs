...
public class KingSlimeSkillComponent : MonsterSkillComponent {
    ...
    public override void Initialize(Transform origin, Condition condition) {
        ...
        _hitBox.GetComponent<Split>().Init(_monster, _condition);
    }
    ...
    public override void Split() {
        if (_monsterStats.HeatPoint < _monsterStats.MaxHeatPoint / 2) {
            _hitBox.GetComponent<Split>().SplitKingSlime();
            _isHalfScale = true;
        }
    }
    public override bool UseSkill() {
        ...
        if (!_target) return false; //타겟이 없으면 리턴
