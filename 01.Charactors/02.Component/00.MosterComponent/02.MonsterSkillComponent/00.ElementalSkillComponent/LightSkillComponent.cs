...
public class LightSkillComponent : DefaultSkillComponent {
    ...
    public override void Initialize(Transform origin, Condition condition) {
        ...
        LearnLightSkill(_monsterStats, _monster);
    }
    public override void LearnLightSkill(Character.Stats monsterStats, Transform monster) {
        if (this.ActiveSkills == null) this.ActiveSkills = new int[3];
        if (this.AcquiredSkills == null) this.AcquiredSkills = new List<int>();
        ////랜덤으로 빛속성 스킬 습득(패시브면 1번만 쓰고 사라짐 액티브면 저장)
        ...
        switch (randomSkillnum) {
            case 0: // default Skills
                ...
                break;
            case 1:
                switch (Random.Range(SkillKind.kLightTmp1, SkillKind.kLightTmp1 + 1))//추후 수정
                {
                    //LightSkills - Active
                    case SkillKind.kLightTmp1:
                        this.ActiveSkills[SkillCount] = SkillKind.kLightTmp1; this.SkillCount++;
                        AcquiredSkills.Add(SkillKind.kLightTmp1);
                        Debug.Log("빛속성 스킬을 배웠습니다"); break;
                }
                break;
        }
        CheckOverlap(SkillKind.kLightTmp1, SkillKind.kLightTmp1); //추후 수정
    }
    ...
    public override bool UseSkill()
    {
        ...
        switch (ActiveSkills[randomIdx])
        {
            ...
            //Light Active
            case SkillKind.kLightTmp1: break;
